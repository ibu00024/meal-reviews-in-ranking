...
const NavigationBar = () => {
    ...
    const fetchSuggestions = async (query: string): Promise<SearchSuggestion[]> => {
        try {
            const response = await fetch("http://localhost:8000/restaurant/search?name=" + query);
            if (!response.ok) {
    ...
    return (
        <div className="search-bar-container">
            <img src={Logo} alt="Meal Review Logo" className="logo" onClick={onClickHome} />
            <SearchBar query={searchQuery} onSearch={setSearchQuery} searchSuggestions={suggestions} setSuggestions={setSuggestions} />
