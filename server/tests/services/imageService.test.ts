...
beforeEach(() => {
    ...
    imageService = new ImageService(imageRepositoryMockInstance, configMockInstance);
})

describe('ImageService', () => {
    describe('isUrlValid', () => {
        test('Should return true when url is valid', async () => {
            when(imageRepositoryMock.isObjectExist(anything())).thenReturn(Promise.resolve(true));
            expect(await imageService.isUrlValid("http://127.0.0.1:8000/test_bucket/image.jpg")).toBe(true);
        })
    })
